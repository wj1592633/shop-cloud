...
(
   ...
   user_id              varchar(36) comment '用户id',
   sku_id               varchar(36),
   take_time            timestamp comment '下单时间',
...
alter table tb_rab_order_goods comment '商品模块的下单rabbit辅助表';
...
alter table tb_rab_order_oder comment '订单模块的下单rabbit辅助表，防重';
...
alter table tb_stop_repeat comment '下单前,防止重复秒杀表';
